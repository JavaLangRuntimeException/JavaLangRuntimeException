...
export function ConfirmModal({
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 p-3 overflow-y-auto overscroll-none backdrop-blur-sm">
      <div className="w-full max-w-lg overflow-hidden rounded-2xl border border-white/20 bg-white/95 shadow-2xl max-h-[70vh] sm:max-h-[75vh] flex flex-col backdrop-blur">
        <div className="flex items-center gap-2 border-b border-white/20 bg-gradient-to-r from-blue-500/90 to-indigo-500/90 px-5 py-3 flex-shrink-0 backdrop-blur">
          <NotebookText className="h-5 w-5 text-white" aria-hidden="true" />
          <h3 className="text-base font-semibold text-white drop-shadow">最終確認</h3>
        </div>
        <div className="p-5 overflow-y-auto overscroll-contain flex-1 min-h-0">
          <div className="grid grid-cols-2 gap-3 text-sm text-zinc-900">
            <div className="rounded-lg border border-white/20 bg-white/90 p-3 shadow-lg backdrop-blur">
            ...
            <div className="rounded-lg border border-white/20 bg-white/90 p-3 col-span-2 shadow-lg backdrop-blur">
            ...
            <div className="rounded-lg border border-white/20 bg-white/90 p-3 col-span-2 shadow-lg backdrop-blur">
              ...
              <div className="mt-1 font-medium">
                ...
                {details.contactMethod === "discord" && (
                  <span className="ml-2 inline-flex items-center rounded-full bg-indigo-100 px-2 py-0.5 text-xs text-indigo-700">Discord名: {details.discordName || "(必須)"}</span>
                )}
                {details.contactMethod === "slack" && (
                  <span className="ml-2 inline-flex items-center rounded-full bg-indigo-100 px-2 py-0.5 text-xs text-indigo-700">Slack名: {details.slackName || "(必須)"}</span>
                )}
                {details.contactMethod === "other" && details.otherNote && (
                  <span className="ml-2 inline-flex items-center rounded-full bg-indigo-100 px-2 py-0.5 text-xs text-indigo-700">備考: {details.otherNote}</span>
                )}
              </div>
            </div>
            <div className="rounded-lg border border-white/20 bg-white/90 p-3 col-span-2 shadow-lg backdrop-blur">
            ...
            {showDetail && details.meetingNote && (
              <div className="rounded-lg border border-white/20 bg-white/90 p-3 col-span-2 shadow-lg backdrop-blur">
                ...
                <div className="mt-1 whitespace-pre-wrap text-[13px] leading-relaxed text-zinc-800 max-h-12 overflow-y-auto pr-1 break-all">{details.meetingNote}</div>
              </div>
            )}
          </div>
