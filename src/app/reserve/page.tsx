...
export default function ReservePage() {
  ...
  const { setReserveLoading } = useLoading();
  ...
  const preservedSlotTextRef = React.useRef("");
          ...
          if (!isOverlappingBusy(t, end, weekBusy)) {
            ...
            if (!cancelled) {
              ...
              if (!preservedSlotTextRef.current) {
                preservedSlotTextRef.current = text;
              }
              setNextAvailableSlotText(text);
            }
            break;
          }
          t = end;
  ...
  const applyNextAvailableSlot = React.useCallback(() => {
    const slotText = preservedSlotTextRef.current || nextAvailableSlotText;
    if (!slotText) return;
    // Format: YYYY/MM/DD(曜) HH:MM〜HH:MM
    const m = slotText.match(/^(\d{4})\/(\d{2})\/(\d{2})\(.+\)\s+(\d{2}):(\d{2})〜(\d{2}):(\d{2})$/);
    if (!m) return;
    const y = Number(m[1]);
    const mo = Number(m[2]);
    const d = Number(m[3]);
    ...
    setNotify(`予約日時を ${slotText} にセットしました`);
    try {
      // Auto hide
    ...
    <HeroBackground images={bgImages} intro={{ enabled: false }}>
      ...
      <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <p className="text-sm text-white/80 flex-1">
          お問い合わせは<a className="underline" href="/contact">Contact</a>ページ・X(<a className="underline" href="https://x.com/JavaLangRuntime" target="_blank" rel="noreferrer">@JavaLangRuntime</a>)でも承っております
        </p>
        <div className="flex flex-col items-end">
          ...
          <button
            type="button"
            className="mt-2 rounded-lg px-4 py-2 text-sm font-semibold transition focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-transparent bg-gradient-to-r from-red-600 to-rose-600 text-white shadow-md shadow-red-500/20 hover:from-red-500 hover:to-rose-500 touch-manipulation"
            onClick={() => {
              setCancelModalEventId("");
      ...
      <div className="mt-4 sm:mt-6">
        {slotSuggestLoading ? (
        ...
        ) : (preservedSlotTextRef.current || nextAvailableSlotText) ? (
          <GlassCardSimple
            gradientFrom="from-blue-400"
            gradientTo="to-indigo-500"
            iconColor="text-blue-300"
          ...
          >
            <div className="flex items-center justify-between gap-3">
              <p className="m-0 flex-1 text-sm text-white/90">直近相談予約可能時間(30分枠): <span className="font-semibold text-white">{preservedSlotTextRef.current || nextAvailableSlotText}</span></p>
              <button
                type="button"
      ...
      <section className="mt-6 grid gap-4 sm:grid-cols-2">
        <div className="rounded-2xl border border-white/20 bg-white/90 p-6 shadow-lg backdrop-blur sm:col-span-2 animate-in fade-in-50">
          ...
          <div className="mb-4" />
            ...
            <ul className="list-disc space-y-1 text-xs text-zinc-700">
              入力いただいたメールアドレスに Google カレンダーから招待が届きます。お手数ですが必ずご確認ください。<br />
        ...
        {contactMethod === "offline" && (
          <div className="sm:col-span-2 -mb-2">
            <GlassCardSimple
              ...
              animationDelay={0}
            >
              <p className="m-0 text-xs text-white/90">オフライン面談の予約可能時間は 10:00 - 21:00 です。</p>
            </GlassCardSimple>
          </div>
        )}
      </section>
      ...
      <CancelModal
        ...
        onDeleteSuccess={() => {
          // 削除成功時：完了モーダルも閉じてページをリロード
          setCreatedInfo(null);
          setCompletedDetails(null);
          if (typeof window !== "undefined") window.location.reload();
        }}
        initialEventId={cancelModalEventId}
      />
