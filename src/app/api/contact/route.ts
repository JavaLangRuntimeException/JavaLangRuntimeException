...
async function sendEmail(to: string, subject: string, text: string, attachments: Array<{filename: string, content: Buffer, contentType: string}> = []) {
  // メールアドレスの検証
  if (!to || to.trim() === "") {
...
// AWS SESを使用したメール送信
async function sendEmailWithSES(to: string, subject: string, text: string, attachments: Array<{filename: string, content: Buffer, contentType: string}>, awsAccessKeyId: string, awsSecretAccessKey: string, awsRegion: string) {
  try {
    ...
    // MIMEメールを作成
    const boundary = '----=_NextJS_Contact_Form_Boundary';
    let rawMessage = '';
